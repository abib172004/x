  ...
  return (
    <div className="ecran-appareils">
      <div className="en-tete-appareils">
        ...
        <button className="bouton-primaire">Appairer un nouvel appareil</button>
      </div>
            ...
            {appareils.length > 0 ? (
              appareils.map(appareil => (
                <tr key={appareil.id}>
                  <td>{appareil.nom}</td>
                  <td>{appareil.type}</td>
                  <td>{new Date(appareil.date_appairage).toLocaleDateString('fr-FR')}</td>
                  <td>
                    <span className={`pastille-statut statut-${appareil.statut.toLowerCase()}`}>
                      {appareil.statut}
                    </span>
                  </td>
                  <td>
                    <button
                      ...
                      onClick={() => revoquerAppareil(appareil.id)}
                    >
                      Révoquer
                    </button>
                  </td>
